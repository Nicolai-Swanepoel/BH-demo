...
/* ========= Team page: tap-to-toggle profile on touch devices ========= */
(() => {
  const members = document.querySelectorAll(".tmember:not(.tmember--static)");
  if (!members.length) return;

  // Only intercept taps where hover is unavailable (touch). On hover-capable
  // devices the CSS :hover handles everything and clicks do nothing special.
  ...
  members.forEach((card) => {
    card.addEventListener("click", () => {
      ...
      document.querySelectorAll(".tmember.is-open").forEach((c) => c.classList.remove("is-open"));
  ...
  // Tap outside any card closes an open one
  document.addEventListener("click", (e) => {
    if (!e.target.closest(".tmember")) {
      document.querySelectorAll(".tmember.is-open").forEach((c) => c.classList.remove("is-open"));
    }
  });
})();
